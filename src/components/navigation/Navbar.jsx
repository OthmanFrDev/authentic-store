...
export default function Navbar() {
    ...
    return (
        <div className='flex justify-between items-center h-24 max-w-[1240px] mx-auto px-4 text-black'>
            ...
            {menu.items.map(item=><li key={item} className='p-4 whitespace-nowrap'>{item}</li>)}
            </ul>
            ...
            <div className={nav ? 'fixed left-0 top-0 w-[60%] z-40 h-full border-r border-r-gray-900 bg-[#E7E7E7] ease-in-out duration-500' : 'fixed left-[-100%]'}>
                ...
                <ul className='p-4 uppercase'>
                    {menu.items.map(item=><li key={item} className='p-4 border-b border-gray-600 whitespace-nowrap'>{item}</li>)}
                </ul>
            </div>
        </div>
    )
